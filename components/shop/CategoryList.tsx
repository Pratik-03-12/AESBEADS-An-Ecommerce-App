      ...
      <RadioGroup value={selectedCategory || ""} onValueChange={setSelectedCategory} className='mt-2 space-y-1'>
        {categories?.map((category)=>(
          <div key={category?._id} className='flex items-center space-x-2 hover:cursor-pointer'>
            ...
            <Label htmlFor={category?.slug?.current} className={`${selectedCategory === category?.slug?.current ? "font-semibold text-shop-warmterracotta":"font-normal"}`}>{category?.title}</Label>
          </div>
        ))}
      </RadioGroup>
      {selectedCategory && (<button onClick={()=>setSelectedCategory(null)} className='text-sm font-medium mt-2 underline underline-offset-2 decoration-[1px] hover:text-shop-warmterracotta hoverEffect text-left'>
        Reset Selection
      </button>)}
    </div>
  );
